...
import { Sparkles, FileText, CheckCircle, ArrowRight, ShieldCheck, ListFilter } from 'lucide-react';

export const metadata: Metadata = {
  title: "Resume Job Description Matcher & Gap Analyzer | CareerSignal AI",
  description: "Match your resume to any job description online. Identify missing skills, core requirements, and score your alignment before applying.",
  keywords: ["Job description matcher", "Match resume to job description", "Resume skills gap analyzer", "Resume matching tool", "ATS match rate checker"],
};
  ...
  return (
    <div className="min-h-screen text-[var(--text-1)] relative overflow-hidden flex flex-col justify-between">
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-5 py-12 md:py-20 space-y-20 relative z-10">
        ...
        <section className="text-center space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/8 bg-white/[0.02] text-[11px] text-[var(--text-2)]">
            ...
            Real-time JD Matching Calibration
          </div>
          ...
          <p className="text-[var(--text-2)] text-base sm:text-lg leading-relaxed max-w-2xl mx-auto">
            Recruiters scan resumes in 6 seconds looking for key technical skills and domain alignments. Paste your target job description to check your match rate instantly.
          </p>
        ...
        <section className="space-y-8">
          <div className="text-center space-y-2">
            <h2 className="text-2xl font-bold text-white">How does Job Description Matching work?</h2>
            <p className="text-xs text-[var(--text-3)] uppercase tracking-wider font-semibold">Our 3-step alignment analysis</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="card p-6 space-y-3 bg-white/[0.01]">
              ...
              <h3 className="text-sm font-bold text-white">1. Skill Gap Finder</h3>
              <p className="text-xs text-[var(--text-2)] leading-relaxed">
                We read the target job description to build a custom rubric of required, preferred, and core technology stack credentials.
              </p>
            </div>
            ...
            <div className="card p-6 space-y-3 bg-white/[0.01]">
              ...
              <h3 className="text-sm font-bold text-white">2. Red Flag Warnings</h3>
              <p className="text-xs text-[var(--text-2)] leading-relaxed">
                Identify mismatches in seniority, missing core platform proficiencies, or structural issues that would fail automated criteria checks.
              </p>
            </div>
            ...
            <div className="card p-6 space-y-3 bg-white/[0.01]">
              ...
              <h3 className="text-sm font-bold text-white">3. Tailored Rewrites</h3>
              <p className="text-xs text-[var(--text-2)] leading-relaxed">
                Generate highly optimized rewrites for your current bullets to align them with the specific phrasing used in the job description.
              </p>
            </div>
          </div>
        </section>
      ...
      <footer className="border-t border-white/5 bg-zinc-950 py-6 text-center text-xs text-[var(--text-3)]">
        <p>&copy; {new Date().getFullYear()} CareerSignal AI. Match your profile to any job description online.</p>
      </footer>
    </div>
  );
}
